...
type NewTables = {
    [k in keyof PostgresTables]: {
        Row: k extends keyof TableExtensions ? TakeDefinitionFromSecond<PostgresTables[k]['Row'], TableExtensions[k]> : PostgresTables[k]['Row'];
        Insert: k extends keyof TableExtensions ? TakeDefinitionFromSecond<PostgresTables[k]['Insert'], TableExtensions[k]> : PostgresTables[k]['Insert'];
        Update: k extends keyof TableExtensions ? Partial<TakeDefinitionFromSecond<PostgresTables[k]['Update'], TableExtensions[k]>> : PostgresTables[k]['Update'];
        Relationships: PostgresTables[k]['Relationships'];
    };
};
type NewViews = {
    [k in keyof PostgresViews]: {
        Row: k extends keyof ViewExtensions ? TakeDefinitionFromSecond<PostgresViews[k]['Row'], ViewExtensions[k]> : PostgresViews[k]['Row'];
...
type NewFunctions = {
    [K in keyof PostgresFunctions]: {
        ...
        Returns: K extends keyof FunctionExtensions ? PostgresFunctions[K]['Returns'] extends (infer Row)[] ? Row extends object ? TakeDefinitionFromSecond<Row, FunctionExtensions[K]>[] : PostgresFunctions[K]['Returns'] : PostgresFunctions[K]['Returns'] : PostgresFunctions[K]['Returns'];
    };
};
